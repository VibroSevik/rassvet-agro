...
class ControllerExtensionThemeNice extends Controller {
	...
	public function install() {
		...
		// ? What about modificator of mega menu?
		// todo...		
		...
		$this->load->model('extension/theme/nice');
		
		$values = $this->model_extension_theme_nice->defaultValues();
		
		foreach ($values as $key => $value) {
			$this->model_extension_theme_nice->addSettingValue('theme_nice', $key, $value);
		}
		
		// Make Nice Theme for config
		$this->load->model('setting/setting');

		$this->model_setting_setting->editSettingValue('config', 'config_theme', 'nice');
		
		// Refresh modificators		
		$this->load->controller('marketplace/modification/refresh');
	}
	
	/*
	 * A! It is called after system method (!)
	 * So I cannot get old settings here
	 */
	public function uninstall() {
		// Dectivate this theme in settings	
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSettingValue('config', 'config_theme', 'default');

		// Disable old modificators
		...
		$modification = $this->model_setting_modification->getModificationByCode('Nice_MegaMenu'); // A! Not Extension Code!
		$this->model_setting_modification->disableModification($modification['modification_id']);
		
		$this->removeOldModificators();
		
		// Refresh modification
		$this->load->controller('marketplace/modification/refresh');		
	}
	
	public function update_from_1_5_to_1_6() {
		// Disable old modificators
		$this->load->model('setting/modification');
		...
		$this->load->model('extension/theme/nice');		
		$values = $this->model_extension_theme_nice->defaultNewValuesIn_1_6_0();		
		foreach ($values as $key => $value) {
			$this->model_extension_theme_nice->addSettingValue('theme_nice', $key, $value);
		}
		...
		// It shows login form...
		//$c = file_get_contents($this->url->link('marketplace/modification/refresh', 'user_token=' . $this->session->data['user_token'] . '&store_id=0'));
		//echo $c;		
		
		// It show modificator page even if use ob_start();
		// So I hide #container in CSS		
		$this->load->controller('marketplace/modification/refresh');
		
		// Save old settings and add new - for why???
//		$this->load->model('setting/setting');
//		$this->load->model('extension/theme/nice');
//		
//		$nice_settings_old = $this->model_setting_setting->getSetting('theme_nice', 0);
//		
//		$values = $this->model_extension_theme_nice->defaultValues();
//		
//		if (isset($nice_settings_old) && is_array($nice_settings_old)) {
//			$values = array_merge($nice_settings_old, $values);
//		} else {
//			$values = $this->model_extension_theme_nice->defaultValues();
//		}
//		
...
//		}		
		...
		echo '<div style="width: 600px; margin-left: auto; margin-right: auto; text-align: center;">';
		echo '<br><br><br><br><br><br><b>Update is complete!</b><br><br>';
		echo '<p><a href="' . $this->url->link('extension/theme/nice', 'user_token=' . $this->session->data['user_token'] . '&store_id=0') . '" style="" >>>> Click here to continue <<<</a></p><br><br><br><br><br><br>';
		echo '</div>';
		
	}
	
	private function removeOldModificators() {
		$old_modificators = [
			'nice-theme-productlist.ocmod.xml',
			'nice-theme-megamenu.ocmod.xml',
		];

		foreach ($old_modificators as $file) {
			if (is_file(DIR_SYSTEM . $file)) {
	...
	public function copySettings() {
		$this->load->model('setting/setting');
		$this->session->data['nice_settings_old'] = $this->model_setting_setting->getSetting('theme_nice', 0);
		echo '<b>Setting are copied!</b><br>';
		echo '<p><a href="' . $this->url->link('extension/theme/nice', 'user_token=' . $this->session->data['user_token'] . '&store_id=0') . '">>>> Click here to continue <<<</a></p>';
		
		echo '<pre>';
		var_export($this->session->data['nice_settings_old']);
		echo '</pre>';
	}
	...
	public function index() {
		...
		if (isset($this->request->post['theme_nice_subscribe_email'])) {
			...
			$data['theme_nice_subscribe_email'] = $setting_info['theme_nice_subscribe_email'];
		} else {
			$data['theme_nice_subscribe_email'] = '';
		}
		...
		if (isset($this->request->post['theme_nice_color_menu_bg'])) {
			$data['theme_nice_color_menu_bg'] = $this->request->post['theme_nice_color_menu_bg'];
		} elseif (isset($setting_info['theme_nice_color_menu_bg'])) {
			$data['theme_nice_color_menu_bg'] = $setting_info['theme_nice_color_menu_bg'];
		} else {
			$data['theme_nice_color_menu_bg'] = '#6667ab';
		}
		
//		if (isset($this->request->post['theme_nice_color_menu_bg__hover'])) {
//			$data['theme_nice_color_menu_bg__hover'] = $this->request->post['theme_nice_color_menu_bg__hover'];
//		} elseif (isset($setting_info['theme_nice_color_menu_bg__hover'])) {
//			$data['theme_nice_color_menu_bg__hover'] = $setting_info['theme_nice_color_menu_bg__hover'];
//		} else {
//			$data['theme_nice_color_menu_bg__hover'] = '#7a7bb6';
//		}
		
		if (isset($this->request->post['theme_nice_color_menu_bg__mobile'])) {
			$data['theme_nice_color_menu_bg__mobile'] = $this->request->post['theme_nice_color_menu_bg__mobile'];
		} elseif (isset($setting_info['theme_nice_color_menu_bg__mobile'])) {
			$data['theme_nice_color_menu_bg__mobile'] = $setting_info['theme_nice_color_menu_bg__mobile'];
		} else {
			$data['theme_nice_color_menu_bg__mobile'] = '#f8f8f8';
		}
		...
		if (isset($this->request->post['theme_nice_menu_top_mobile_label'])) {
			$data['theme_nice_menu_top_mobile_label'] = $this->request->post['theme_nice_menu_top_mobile_label'];
		} elseif (isset($setting_info['theme_nice_menu_top_mobile_label'])) {
			$data['theme_nice_menu_top_mobile_label'] = $setting_info['theme_nice_menu_top_mobile_label'];
		} else {
			$data['theme_nice_menu_top_mobile_label'] = '';
		}
		...
		/* Tab Header
		 ------------------------------------------------------------------------ */
		// Tab Header Added in version 1.6.0
		if (isset($this->request->post['theme_nice_mobile_logo_center'])) {
			$data['theme_nice_mobile_logo_center'] = $this->request->post['theme_nice_mobile_logo_center'];
		} elseif (isset($setting_info['theme_nice_mobile_logo_center'])) {
			$data['theme_nice_mobile_logo_center'] = $setting_info['theme_nice_mobile_logo_center'];
		} else {
			$data['theme_nice_mobile_logo_center'] = false;
		}
		
		if (isset($this->request->post['theme_nice_multilang_logo_status'])) {
			$data['theme_nice_multilang_logo_status'] = $this->request->post['theme_nice_multilang_logo_status'];
		} elseif (isset($setting_info['theme_nice_multilang_logo_status'])) {
			$data['theme_nice_multilang_logo_status'] = $setting_info['theme_nice_multilang_logo_status'];
		} else {
			$data['theme_nice_multilang_logo_status'] = false;
		}
		
		if (isset($this->request->post['theme_nice_multilang_logo_type'])) {
			$data['theme_nice_multilang_logo_type'] = $this->request->post['theme_nice_multilang_logo_type'];
		} elseif (isset($setting_info['theme_nice_multilang_logo_type'])) {
			$data['theme_nice_multilang_logo_type'] = $setting_info['theme_nice_multilang_logo_type'];
		} else {
			$data['theme_nice_multilang_logo_type'] = false;
		}
		
		foreach ($data['languages'] as $language) {
			if (isset($this->request->post['theme_nice_multilang_logo_image'][$language['language_id']]) && is_file(DIR_IMAGE . $this->request->post['theme_nice_multilang_logo_image'][$language['language_id']])) {
				$data['theme_nice_multilang_logo_image'][$language['language_id']]	= $this->request->post['theme_nice_multilang_logo_image'][$language['language_id']];
				$data['thumb_logo'][$language['language_id']] = $this->model_tool_image->resize($this->request->post['theme_nice_multilang_logo_image'][$language['language_id']], 100, 100);
			} elseif (isset($setting_info['theme_nice_multilang_logo_image'][$language['language_id']]) && is_file(DIR_IMAGE . $setting_info['theme_nice_multilang_logo_image'][$language['language_id']])) {
				$data['theme_nice_multilang_logo_image'][$language['language_id']] = $setting_info['theme_nice_multilang_logo_image'][$language['language_id']];
				$data['thumb_logo'][$language['language_id']] = $this->model_tool_image->resize($setting_info['theme_nice_multilang_logo_image'][$language['language_id']], 100, 100);
			} else {
				$data['theme_nice_multilang_logo_image'][$language['language_id']] = '';
				$data['thumb_logo'][$language['language_id']] = $this->model_tool_image->resize('no_image.png', 100, 100);
			}
		}

		$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		
		if (isset($this->request->post['theme_nice_multilang_logo_text_icon'])) {
			$data['theme_nice_multilang_logo_text_icon'] = $this->request->post['theme_nice_multilang_logo_text_icon'];
		} elseif (isset($setting_info['theme_nice_multilang_logo_text_icon'])) {
			$data['theme_nice_multilang_logo_text_icon'] = $setting_info['theme_nice_multilang_logo_text_icon'];
		} else {
			$data['theme_nice_multilang_logo_text_icon'] = '';
		}
		
		if (isset($this->request->post['theme_nice_multilang_logo_text'])) {
			$data['theme_nice_multilang_logo_text'] = $this->request->post['theme_nice_multilang_logo_text'];
		} elseif (isset($setting_info['theme_nice_multilang_logo_text'])) {
			$data['theme_nice_multilang_logo_text'] = $setting_info['theme_nice_multilang_logo_text'];
		} else {
			$data['theme_nice_multilang_logo_text'] = $empty_array_with_languages_keys;
		}
		
		if (isset($this->request->post['theme_nice_search_categories_status'])) {
			$data['theme_nice_search_categories_status'] = $this->request->post['theme_nice_search_categories_status'];
		} elseif (isset($setting_info['theme_nice_search_categories_status'])) {
			$data['theme_nice_search_categories_status'] = $setting_info['theme_nice_search_categories_status'];
		} else {
			$data['theme_nice_search_categories_status'] = false;
		}
		...
		/* Tab Custom CSS
		 ------------------------------------------------------------------------ */
		if (isset($this->request->post['theme_nice_custom_css'])) {
			$data['theme_nice_custom_css'] = $this->request->post['theme_nice_custom_css'];
		} elseif (isset($setting_info['theme_nice_custom_css'])) {
			$data['theme_nice_custom_css'] = $setting_info['theme_nice_custom_css'];
		} else {
			$data['theme_nice_custom_css'] = '';
		}
...
//		echo "\$css_file = $css_file<br>";
		...
		$replace = [
			...
			$this->request->post['theme_nice_color_menu_bg'],
			$this->request->post['theme_nice_color_menu_bg__mobile'],
...
//		echo $css_colors_result;		
		...
		// Custom CSS
		$pattern = "/\/\*\s*Nice\s*Custom\s*CSS\s*\.\s*Begin(.*?)\s*Nice\s*Custom\s*CSS\s*\.\s*End\s*\*\//s";
		
		$custom_css = "/* Nice Custom CSS . Begin */\r\n/* Warning!\r\n * Not remove these comments!\r\n * It is neccessary for corect saving of Custom CSS from admin part */\r\n"
			. $this->request->post['theme_nice_custom_css'] 
			. "\r\n/* Nice Custom CSS . End */";
		
		$css = preg_replace($pattern, $custom_css, $css);
	...
	private function notify() {
		$url = 'https://api.sergetkach.com/notify/nice';
	...
	public function subscribe() {
		...
		$json = [
			'status'	=> 'Success',
			'message' => $this->language->get('subscribe_message_ok'),
		];
		
		if (empty($this->request->post['email'])) {
			$json = [
				'status'	 => 'Error',
				'message'	 => $this->language->get('subscribe_error_email_empty'),
			];
			
			goto subscribe_output;
		}
		
		if (!filter_var($this->request->post['email'], FILTER_VALIDATE_EMAIL)) {
			$json = [
				'status'	 => 'Error',
				'message'	 => $this->language->get('subscribe_error_email_not_valid'),
			];
		...
		$data = [
			...
			'language' => $this->language->get('subscribe_language'),
		];		
		
		$response = $this->subscribeByAPI($data);
		
		if (NULL === $response) {
			$json = [
				'status'	 => 'Error',
				'message'	 => $this->language->get('subscribe_message_error'),
			];
		} else {
			if ('Error' == $response['status']) {
				$json = [
					'status'	 => 'Error',
					'message'	 => $this->language->get('subscribe_error_' . $response['message']),
				];
			} else {
				// save statement
				$this->model_extension_theme_nice->addSettingValue('theme_nice', 'theme_nice_subscribe_email', $this->request->post['email']);
			}
		}
	...
	private function subscribeByAPI($data) {
		$url = 'https://api.sergetkach.com/subscribe/nice';
